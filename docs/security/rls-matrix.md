...
**Wersja:** v0.8 · 2026-05-28
**Owner:** Ethan (CTO)
**Status:** **Wdrożone i podpisane** — sign-off Ryana (CRCO, domena 8 / G6) wystawiony 2026-05-27 (`docs/security/beta-v01-signoff.md`, werdykt GO). **R1/R2 doc-closed** (ADR-002 + ADR-004); migracje X/Y czekają na landing kolumn. **§8 #4 ZAMKNIĘTE** (migracja `0010`, TRUNCATE protection). **§8 #5 ZAMKNIĘTE** (rotacja share_token na disable). **§8 #1 Phase 1 ZAMKNIĘTE** — migracja `0011` tworzy rolę `app_runtime` (NOLOGIN, NOBYPASSRLS) jako członek `app_student`/`app_faculty`; `dbRuntime` w `src/lib/db/index.ts` używa `DATABASE_URL_RUNTIME` (fallback do owner z warningiem); `withTenantContext` przełączone na `dbRuntime`; `k3-validate` test 9/9 (rola istnieje, NOBYPASSRLS, membership w obu grupach). **§8 #1 Phase 2 = sub-issues**: ops-LOGIN+ENV (19a), refactor 6 tras studenta na `withTenantContext` (19b…19g, 1 PR per trasa), FORCE RLS z ADR-005 (19h, wymaga design seed-handling), wpięcie `k3-validate` do CI z ephemeral DB (19i).
**Wejście:** `schema.ts` (`main`, **16 tabel** po dropie micro-courses w `0004`) · ADR-001 sekcja 4.2 (multi-tenancy + RLS) · ADR-003 (strategia: WHERE primary + RLS defense-in-depth, 4 warstwy) · `docs/audyty/2026-05-20-skillbridge-ai-production-readiness.md` (K3) · ADR-008 (drop micro-courses — WYKONANY) · **ADR-002** (`docs/decisions/002-column-level-isolation-r1.md`, column-level isolation R1, `Accepted` 2026-05-28) · **ADR-004** (`docs/decisions/004-faculty-update-column-isolation-r2.md`, column-level isolation R2 — write-side, `Accepted` 2026-05-28).
...
1. **`FORCE RLS` + nie-właścicielska rola login.**
	- **Phase 1 — ZAMKNIĘTE 2026-05-28 (migracja `0011_app_runtime_role.sql`):** rola `app_runtime` NOLOGIN, NOBYPASSRLS, członek `app_student`/`app_faculty`. `dbRuntime` w `src/lib/db/index.ts` z `DATABASE_URL_RUNTIME` env (fallback do `DATABASE_URL` z warningiem). `withTenantContext` przełączone na `dbRuntime` — gdy ops ustawi `DATABASE_URL_RUNTIME`, runtime przechodzi na `app_runtime` bez zmiany w kodzie. `k3-validate` test #9 weryfikuje rolę + NOBYPASSRLS + membership. **Co to daje dziś:** fundament gotowy, ścieżki przez `withTenantContext` (`faculty/dashboard`) egzekwują RLS jak dotąd; po ops-step (Phase 2a) — egzekwują też defense-in-depth bez owner-bypass dla zapomnianych `SET LOCAL ROLE`.
	- **Phase 2 — sub-issues (po Becie, każdy własny PR):**
	  - **19a:** Ops: `ALTER ROLE app_runtime LOGIN PASSWORD '<gen>'` w Neon + ustawienie `DATABASE_URL_RUNTIME` w Vercel (preview + prod). Bez kodu.
	  - **19b…19g:** Refactor 6 tras studenta (`onboarding`, `skill-map`, `gaps`, `gaps/[id]/why`, `projects/[id]/brief`, `projects/[id]/submit`, `passport*`) z `db` bezpośrednio na `withTenantContext({userId, tenantId, role: "student"})`. Każda trasa = własny PR (zachowanie behaviour-preservation łatwiejsze do review per route). Wzorzec: jak `src/app/api/faculty/dashboard/route.ts`.
	  - **19h:** `ALTER TABLE … FORCE ROW LEVEL SECURITY` na 6 tabelach studenta + ADR-005 dla seed/migrations handling (FORCE wymaga, żeby seed.ts był `SET LOCAL ROLE app_student` per insert albo polityka owner-passthrough — Ethan analizuje seed.ts przed wyborem).
	  - **19i:** Wpięcie `tools/k3-validate.ts` do GitHub Actions (job `integration` po `pnpm db:migrate` + nowy seed-step + uruchomienie skryptu z testami 9/9).
