...
**Wersja:** v0.9 · 2026-05-28
**Owner:** Ethan (CTO)
**Status:** **Wdrożone i podpisane** — sign-off Ryana (CRCO, domena 8 / G6) wystawiony 2026-05-27. **R1/R2 doc-closed** (ADR-002 + ADR-004); migracje X/Y czekają na landing kolumn. **§8 #4 ZAMKNIĘTE** (TRUNCATE protection). **§8 #5 ZAMKNIĘTE** (rotacja share_token na disable). **§8 #1 Phase 1 ZAMKNIĘTE** (rola `app_runtime` + `dbRuntime` + `withTenantContext` rewire, migracja `0011`). **§8 #1 Phase 2 prawie ZAMKNIĘTE**: 6 refaktorów tras studenta przez `withTenantContext` (sub-issues #19b…#19g, PR-y #35-#40, wszystkie 352/352 testów); **FORCE RLS + ADR-005 ZAMKNIĘTE** (sub-issue #19h, migracja `0012` — `ALTER TABLE … FORCE` na 6 tabelach + `owner_passthrough` policy `TO neondb_owner`; `k3-validate` test 10/10 z deny-default verification). Pozostałe: **#19a (ops LOGIN+ENV — czeka na Darka)** + **#19i (wpięcie k3-validate w CI z seed step — osobny PR)**.
...
1. **`FORCE RLS` + nie-właścicielska rola login.**
	...
	- **Phase 2 sub-issues — status 2026-05-28:**
	  - **19a — OTWARTY (Darek):** `ALTER ROLE app_runtime LOGIN PASSWORD '<gen>'` w Neon + `DATABASE_URL_RUNTIME` w Vercel (preview + prod). Bez kodu, ops only.
	  ...
	  - **19i — OTWARTY (osobny PR):** Wpięcie `tools/k3-validate.ts` do GitHub Actions (job `integration` po `pnpm db:migrate` + seed-step + skrypt z testami 10/10).
