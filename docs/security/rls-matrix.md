...
**Wersja:** v0.5 · 2026-05-28
**Owner:** Ethan (CTO)
**Status:** **Wdrożone i podpisane** — sign-off Ryana (CRCO, domena 8 / G6) wystawiony 2026-05-27 (`docs/security/beta-v01-signoff.md`, werdykt GO). Ta wersja godzi zapis macierzy z **faktycznym wdrożeniem** na gałęzi `feat/k3-rls-multitenancy` (PR #18). Migracja prod = osobna czerwona linia (`docs/runbooks/k3-prod-migration.md`). **R1 (samoocena vs faculty SELECT) i R2 (faculty UPDATE na project_submissions) nazwane i podpisane** w `§8 #2` oraz ADR-002 (R1, read-side) + ADR-004 (R2, write-side), oba `Accepted`, sign-off Ethan + Ryan 2026-05-28 — kierunek (column-level `REVOKE`/`GRANT` na `app_faculty`) zaakceptowany po obu stronach; migracje X (R1) i Y (R2) uruchamiane wraz z dodaniem odpowiadających kolumn (`competencies.self_assessment` / `project_submissions.verified_by` + `verified_at` — dziś żadna nie istnieje).
