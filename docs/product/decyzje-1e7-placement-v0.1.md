...
**Changelog v0.10 → v0.11 (2026-08-01, Sophia):** **1E.7 zapalone na produkcji (decyzja Darka) — a pierwszy wiersz `curriculum_placements` NIE jest studencki i musi wypaść z każdego odczytu miernika.** Rozstrzygnięcia v0.1–v0.10 bez zmian; jedna reguła odczytu dopisana, jedno ograniczenie wnioskowania zaostrzone.
1. **Pierwszy wiersz jest techniczny — pochodzi z weryfikacji zapłonu kontem QA (Ethan, 2026-08-01)**, a nie od uczestnika pilotażu. Wygląda jak **udany** placement (`qualified`, poziom 4, wsparcie wygaszane, dziura `no_measurement` na Pandas, jedno odblokowanie), więc pierwszy człowiek czytający dane zobaczy „jedno odblokowanie na jednego uczestnika" i wyciągnie wniosek, przed którym ostrzega §6a. Zapis: §6a + §13 / D11.
2. **Sposób wykluczenia — sprawdziłam kod i odrzucam oba zaproponowane rozróżniki jako REGUŁĘ PODSTAWOWĄ.** Zdarzenie miernika **nie niesie `actorId`** (wymóg Ryana, art. 17 RODO — `placement-service.ts`, komentarz przy `recordPlacementMetricEvent`): jedynym wiązaniem z osobą jest `targetId` = id sesji diagnozy. Domena `.invalid` i oznaczenie uczelni leżą na koncie, więc dosięgnąć ich można **wyłącznie złączeniem przez `assessment_sessions`** — a to złączenie **przestaje istnieć w chwili skasowania konta** (sesja znika kaskadą, zdarzenie zostaje sierotą, z założenia). Rozróżnik oparty na koncie znika więc dokładnie wtedy, gdy jest potrzebny, i wiersz techniczny **po cichu wraca do licznika**.
3. **Reguła wiążąca odczytu miernika (nowa, §6a):** miernik czyta się **wobec imiennej listy uczestników pilotażu**; zdarzenie, którego nie da się przypisać do sesji **nazwanego uczestnika**, **nie jest obserwacją** — niezależnie od tego, jak wygląda. To rozszerzenie mojej własnej zasady „mianownik z listy uczestników, nie z dziennika" (v0.7) z **mianownika na LICZNIK**, bo zanieczyszczenie trafiło właśnie w licznik.
4. **Dwa potwierdzenia z żywego przebiegu zapisane jako dowód o KODZIE, nie o studentach** (§6a): nadpisanie korzeniem z DECYZJI 3 zadziałało na produkcji, a dziura wypadła na `no_measurement`, więc zdanie „wypadła słabo" się nie pokazało (§12.4 reguła 1). Obie rzeczy zamykają pytania weryfikacyjne i **nie wnoszą ani grama wiedzy produktowej**.

**Changelog v0.9 → v0.10 (2026-08-01, Sophia):** **luka zgłoszona przez Jacka zamknięta poprawką TEKSTU, nie kontraktu + sześć jego rozstrzygnięć przeniesionych z kodu do dokumentu + jedno rozstrzygnięcie, o które prosił w komentarzu, a którego nie było na liście.** Rozstrzygnięcia v0.1–v0.9 bez zmian; jeden tekst §8 **przepisany**, bo był błędny — nie skrócony dla wygody.
1. **Cisza przy `below_threshold` z niezerowymi odblokowaniami: wybieram (B) — przepisuję tekst. Odrzucam (A) jako NIEBEZPIECZNE, nie tylko drogie.** Sprawdziłam regułę w kodzie (`placement.ts`, `resolvePrefixEnd`): dziurą zostaje **pierwszy otagowany moduł, który nie spełnia warunku** — niezależnie od tego, czy cokolwiek głębiej się zakwalifikowało. „Kompetencja głębsza" **nie jest więc gwarantowana przez regułę**: przy `ds-python=4, ds-pandas=1, ds-sql=4` istnieje (SQL), a przy `ds-python=4, ds-pandas=1` i niezaznaczonym SQL — **nie istnieje**, i oba kształty są dziś osiągalne. Czwarte pole nie zlikwidowałoby więc ciszy, tylko **zwęziło ją do podzbioru sesji** — dwóch studentów w tej samej sytuacji dostałoby różne traktowanie, a ja zapłaciłabym za to zmianą kontraktu w ostatniej chwili przed scaleniem. Nowy tekst interpoluje **wyłącznie pola, które kontrakt już niesie**. **Cisza (C) też odpada** i zgadzam się z Twoim domyślnym osądem: to jedyne miejsce, gdzie pytania **realnie padły** i student **realnie** wypadł poniżej progu — czyli jedyne, w którym może sam sprawdzić, że wie więcej, niż mu powiedzieliśmy.
2. **Przy okazji naprawiam przyczynę, nie tylko ten tekst:** reguła interpolacji z v0.8 obejmowała wyłącznie sesje z zerem odblokowań. Rozszerzam ją na **wszystkie** teksty o dziurze i zamykam kategorię „kompetencja głębsza" **na stałe** — nie wolno budować zdania na module innym niż dziura i ostatni odblokowany, bo reguła placementu nie gwarantuje istnienia żadnego innego (§8).
3. **Sześć rozstrzygnięć Jacka ratyfikowanych i przeniesionych do dokumentu** (§8 tekst korzenia, §12.2 rekomendacja, §12.6 warianty, §12.8 kontrakt, §12.10 etykieta dla czytnika ekranu). Wszystkie zgodne z regułami; dwa z nich naprawiają **moje** błędy (literał tytułu w §8, zawężenie `deepestUnlockedTitle`).
4. **[NOWE, spoza listy] Ratyfikuję interpretację kroku 3 algorytmu rekomendacji**, o którą Jack prosił w komentarzu `placement-screen.ts` i której nikt nie zgłosił: nadpisanie korzeniem obowiązuje **tylko gdy korzeń jest kandydatem**. §12.2 doprecyzowane.
5. **Jedna pozycja świadomie NIE naprawiona teraz: D13** — cisza wyjaśnienia u studenta z zaliczeniami i zerem odblokowań (wariant 8). Kształt naprawy zapisany, próg nazwany; powód odłożenia w §13.

**Changelog v0.8 → v0.9 (2026-08-01, Sophia):** **trzy rozstrzygnięcia na pytania Mili do §12 + odpowiedź Oliverowi o progach D5(b) i D11.** Rozstrzygnięcia v0.1–v0.8 bez zmian; luka zakresu w §12 zamknięta **zawężeniem, nie rozszerzeniem**.
1. **Karta wejścia w moduł NIE wchodzi do L6 — §12.4 zawężona do JEDNEJ powierzchni (odznaka na drabinie).** Powód nie jest harmonogramowy: **`supportMode` nie ma dziś ani jednego czytelnika w produkcie** (pisarze: `placement.ts:422`, `placement-service.ts:411`; ograniczenie schematu; testy — i koniec). Drabinka podpowiedzi ADR-018 działa identycznie dla każdego studenta i jest odsłaniana na jego żądanie. Zdanie „zostawiamy pełne wsparcie włączone" opisywałoby więc **decyzję bez skutku** — ta sama klasa błędu co nagłówek „bez skrótów" (D7), tylko tym razem popełniona przeze mnie świadomie. Odkładam jako **§13 / D12**, z progiem przypiętym do pierwszego czytelnika `supportMode`, nie do „potem". **Prostuję przy tym własną przesłankę zlecenia:** populacja, która zobaczy moduł otwarty placementem, **nie jest bliska zeru** — `f1-python-1` otwiera się na samym `ds-python ≥3` (Python: 55% ofert, pozycja 1 listy wyboru), więc `qualified` jest najbardziej prawdopodobnym niezerowym wynikiem pilotażu. Odkładam kartę mimo istniejącej populacji, bo nie ma czego jej zakomunikować.
2. **Nagłówek sekcji na powierzchni A: „Po diagnozie" — tekst mój, dopisany do §8.** Odrzucam „Twoja ścieżka" Mili: nazywa tę sekcję tak samo jak `/curriculum` („Ścieżka nauki"), a pod nagłówkiem stoi lista modułów krótsza niż drabina — czyli podanie **wycinka jako stanu ścieżki**, wprost zakazane w §12.7 pkt 1. Rodzina „punkt startu" odpada osobno (§7). Przy okazji zamykam przyczynę, nie objaw: **etykiety strukturalne tego ekranu (nagłówek, `aria-label`) są mikrocopy i należą do §8** — inaczej ta sama luka wraca przy każdej nowej sekcji.
3. **Blok 3 zostaje TEKSTEM — ale uzasadnienie Mili jest oparte na nieprawdziwym fakcie i prostuję je, żeby nikt nie „naprawił" tego wstecz.** `POST /api/onboarding` (zapis `students.career_goal`) leci **wewnątrz `handleDiagnosisFinished`, przed** wyrenderowaniem kroku 4 (`onboarding-wizard.tsx:429` → `advanceTo(4)`), więc link **nie byłby martwy**. Realne ryzyko jest inne: surowy `<Link>` wyprowadza studenta z kreatora z `onboardingCompleted=false`. Mimo to nie linkuję — powód produktowy w §12.3.
4. **D5(b) i D11 zostają „przed 1. studentem", z doprecyzowanym momentem** („przed pierwszą diagnozą pierwszego uczestnika pilotażu", nie „w trakcie pilotażu") — §13.
...
**Autor:** Sophia (PO, dydaktyka) · **Data:** 2026-07-26 (v0.1–v0.3) · 2026-07-27 (v0.4) · 2026-07-30 (v0.5, v0.6, v0.7) · 2026-07-31 (v0.8) · 2026-08-01 (v0.9, v0.10, v0.11) · **Status:** v0.11 — WIĄŻĄCY KONTRAKT produktowy dla slice'ów L1–L6 funkcji 1E.7 (Ethan — backend, Jack — UI, Mila — ekrany wyniku diagnozy)
...
1. **L6 — ekran wyniku diagnozy wg §7 i §8.** Rekomendacja startu liczona z **obu** źródeł dostępności (najgłębszy dostępny i niezaliczony) oraz mikrocopy w wiążącej kolejności zdań („najpierw jego praca, potem nasza diagnoza"). Bez tego ekran albo cofa studenta, albo przypisuje sobie jego pracę — a to jedyny ekran, na którym student styka się z placementem. **[v0.9] Zakres L6 = DWIE powierzchnie (A: krok 4 kreatora, B: drabina), liczba zamknięta.** Karta wejścia w moduł (`/curriculum/[moduleId]`) **nie należy do L6** — rozstrzygnięcie i uzasadnienie: §12.4, zaksięgowanie: §13 / D12. Zakres L6 po v0.9 jest **węższy** niż zapowiadał v0.8, nie szerszy.
...
- **L6** (Jack + Mila) — ekran wyniku diagnozy, obie powierzchnie; bloker D0 naprawiony (źródłem celu jest sesja diagnozy). Siedem rozjazdów specyfikacji zgłoszonych przy implementacji: sześć ratyfikowanych, jeden (tekst `below_threshold`) przepisany w §8.
- **ZAPŁON 2026-08-01** (decyzja Darka) — flaga `placementDiagnostic` ON na produkcji, pętla zweryfikowana kontem QA. **Pierwszy wiersz `curriculum_placements` i pierwsze zdarzenie miernika są TECHNICZNE** i nie wchodzą do żadnego odczytu — reguła i uzasadnienie: §6a, „Pierwszy wiersz na produkcji jest TECHNICZNY"; wymóg dla zapytania: §13 / D11.
...
- **[NOWE v0.11] czytać PIERWSZEGO wiersza `curriculum_placements` ani pierwszego zdarzenia miernika jako obserwacji** — pochodzą z weryfikacji zapłonu, nie od studenta (blok niżej);
...
### [NOWE v0.11] Pierwszy wiersz na produkcji jest TECHNICZNY — reguła odczytu miernika

**Fakt.** 1E.7 zapalone na produkcji 2026-08-01 (decyzja Darka). Pierwszy wiersz w `curriculum_placements` i pierwsze zdarzenie `curriculum.placement.computed` **pochodzą z weryfikacji zapłonu przeprowadzonej przez Ethana kontem QA** — pełna pętla odegrana po to, żeby udowodnić cztery rzeczy naraz: że flaga jest efektywnie zapalona, że migracja działa na żywym przepływie, że zdarzenie miernika nie niesie identyfikatora studenta (dotąd mieliśmy na to **wyłącznie test**) i że ekran renderuje się zgodnie z §12. Konto zostaje — jest właścicielem wiersza, a skasowanie go skasowałoby kaskadą sam dowód (decyzja Olivera).

**Dlaczego to jest MOJA sprawa, a nie tylko porządkowa.** Ten wiersz wygląda jak **udany** placement: `reason='qualified'`, poziom 4, wsparcie wygaszane, dziura `no_measurement` na Pandas, jedno odblokowanie. Pierwszy człowiek, który otworzy dane pilotażu, zobaczy **jedno odblokowanie na jednego uczestnika** — czyli stuprocentową skuteczność zbudowaną w całości na odegranej sesji. To dokładnie ten wniosek, przed którym ostrzega ta sekcja, tylko postawiony na dowodzie mocniejszym niż przeczucie, więc **trudniejszy do podważenia**.

**Sprawdziłam, jak ten wiersz wykluczyć — i oba oczywiste rozróżniki NIE nadają się na regułę podstawową.** Zdarzenie miernika **nie ma `actorId`** (wymóg Ryana z art. 17 RODO, opisany wprost w `placement-service.ts` przy `recordPlacementMetricEvent`): jedynym wiązaniem z osobą jest `targetId` = identyfikator sesji diagnozy. Domena adresu (`.invalid`) i oznaczenie uczelni leżą **na koncie**, więc sięgnąć po nie można wyłącznie złączeniem `audit_log → assessment_sessions → students`. To złączenie **z założenia znika przy skasowaniu konta** (sesja kasuje się kaskadą, zdarzenie zostaje sierotą — tak zaprojektował to Ryan). Rozróżnik oparty na koncie przestaje więc działać dokładnie w chwili, w której zaczyna być potrzebny, a wiersz techniczny **po cichu wraca do licznika** i już nigdy nie da się go odróżnić od prawdziwego.

**Reguła wiążąca — dla każdego, kto czyta miernik:**

> **Miernik czyta się wobec IMIENNEJ LISTY uczestników pilotażu. Zdarzenie, którego nie da się przypisać do sesji diagnozy nazwanego uczestnika, NIE JEST OBSERWACJĄ** — niezależnie od tego, jak sensownie wygląda. Dotyczy to zarówno mianownika, jak i licznika.

To rozszerzenie zasady, którą zapisałam w v0.7 („mianownik bierzemy z listy uczestników, nie z dziennika") — z **mianownika na licznik**, bo zanieczyszczenie trafiło właśnie w licznik. Reguła nie wymaga żadnego pola, żadnego złączenia i żadnej wartości do uzupełnienia, więc **przeżyje skasowanie konta QA** i każdą kolejną odegraną sesję (weryfikacja po wdrożeniu, demo dla design partnera, szkolenie). Świadomie nie opieram jej na dacie ani na identyfikatorze wpisanym w to zdanie: puste miejsce „do uzupełnienia" jest tym samym rodzajem długu, który sama opisałam w D5.

**Dwa rozróżniki pomocnicze — do sprawdzenia, nigdy jako jedyna podstawa:** (a) adres konta w domenie `.invalid` (zarezerwowanej i nierozwiązywalnej z definicji, więc żaden prawdziwy student jej nie ma) oraz oznaczenie uczelni jako technicznej — **dopóki konto istnieje**; (b) kolejność: to jest **pierwsze** zdarzenie tej akcji w dzienniku, sprzed pierwszej diagnozy pierwszego uczestnika.

**Co ten przebieg realnie dowiódł — i czego NIE dowiódł.** Zamykam przy okazji dwa pytania weryfikacyjne, bo oba dotyczą **kodu**, nie studentów:

- **Nadpisanie korzeniem z DECYZJI 3 działa na produkcji** — rekomendacją został korzeń ścieżki, mimo że diagnoza otworzyła moduł Pythona. Do v0.10 był to mechanizm potwierdzony wyłącznie testem.
- **Dziura wypadła na `no_measurement`, więc zdanie o słabym wyniku się nie pokazało** (§12.4 reguła 1, `placement-summary.tsx`). Reguła, o którą walczyłam najmocniej, zadziałała w pierwszym prawdziwym przebiegu.
- **Kształt przebiegu zgadza się z przewidywaniem z §6a pkt 4** (Python zaznaczony i zdany → otwiera się `f1-python-1`, dziura na `m-pandas` z powodem `no_measurement`). To potwierdza, że **mapa tagów, reguła i ekran są spięte end-to-end** — i **nic** poza tym.

**Czego z tego nie wolno wywieść:** ani częstości, ani skuteczności, ani niczego o progu. Wejścia tej sesji **wybrał człowiek, który wiedział, co chce zobaczyć** — to jest dowód, że mechanizm działa, i zerowy dowód o tym, jak zachowa się student. Zdanie „w pierwszym przebiegu placement otworzył moduł" jest prawdziwe i **nie znaczy nic**.
...
- **[NOWE v0.9] Nagłówek sekcji placementu na powierzchni A (`<h3>`):** „Po diagnozie".
  **Ta sama wartość obowiązuje w `aria-label` sekcji** — czytnik ekranu ma usłyszeć dokładnie to, co widzi wzrok, bez wariantu rozszerzonego. Uzasadnienie wyboru i odrzucenie „Twojej ścieżki": §12.3. Etykiety strukturalne (nagłówki, `aria-label`) należą do tej sekcji na tych samych prawach co zdania narracji — nagłówek ramuje zdania pod sobą, a ramowanie jest treścią.
  **Jawna słabość tego wyboru:** nagłówek jest **czasowy**, a stoi nad blokiem 1, który mówi o pracy sprzed diagnozy. Obroniłam to (nagłówek nazywa chwilę, pierwsze zdanie nazywa właściciela dorobku), ale to najsłabsze z trzech rozstrzygnięć v0.9 i **pierwsze do rewizji** z rozmów pilotażowych — §6a dopuszcza wprost ocenę zrozumienia „otwarte ≠ zaliczone" z rozmowy ze studentem, nie z danych. Koszt zmiany: jedna stała tekstowa.
...
- **Rekomendacja pierwszego kroku (zawsze, gdy `l0-start` niezaliczony) — [POPRAWIONE v0.10]:**
  „Zacznij od **{tytuł modułu korzenia}** — około 15 minut. Bez działającego notebooka nie ruszysz żadnego ćwiczenia z kodem, nawet jeśli materiał znasz."
  **Poprawka za Jackiem: tytuł jest interpolowany, nie wpisany literałem.** W v0.1–v0.9 stało tu wprost „Start: środowisko pracy" — czyli **mój własny tekst łamał moją własną regułę** z §12.8 pkt 2 („tytuły są danymi z ingestu, zmienią się przy pierwszej erracie, a mapa w kodzie zgnije w ciszy"). Jack wziął tytuł z kontraktu i zgłosił odstępstwo od cytatu zamiast wykonać literał — postąpił słusznie i to jedyne dopuszczalne odstępstwo tego rodzaju: **cytat obowiązuje co do zdania, nigdy co do danej, która ma źródło w bazie**. Przy erracie tytułu `l0-start` literał rozjechałby się z drabiną w ciszy, po obu stronach jednego ekranu.
...
- **Placement nie zadziałał z powodu dziury w prefiksie (`below_threshold` — kompetencja zmierzona, wynik poniżej progu) — [PRZEPISANE v0.10]:**
  „**{kompetencja z dziury}** wypadła w teście słabo, a moduł **{tytuł modułu z dziurą}** jest w tej ścieżce fundamentem pod to, co dalej — dlatego otwieramy do **{tytuł ostatniego odblokowanego}**. To wynik dwóch krótkich pytań, nie ocena Ciebie. Znasz materiał tego modułu? Zdaj jego egzamin (**test out**) i przeskocz go."
  **Dlaczego przepisany (a nie: dlaczego skrócony).** Brzmienie v0.1–v0.9 („Twój wynik z **{kompetencja głębsza}** jest wysoki, ale **{kompetencja płytsza}** wypadła słabo") opierało się na polu, **którego reguła placementu nie gwarantuje**. Sprawdziłam w kodzie (`placement.ts`, `resolvePrefixEnd`): dziurą zostaje **pierwszy otagowany moduł, który nie spełnia warunku** — bez względu na to, czy cokolwiek głębiej się zakwalifikowało. Przy `ds-python=4, ds-pandas=1, ds-sql=4` kompetencja głębsza istnieje (SQL); przy `ds-python=4, ds-pandas=1` bez zaznaczonego SQL — **nie istnieje**. Oba kształty są dziś osiągalne, więc stary tekst **musiałby milczeć w części sesji**, i dokładnie to robił (Jack, fail-closed, `placement-summary.tsx`). Gorzej: gdyby ktoś wypełnił to pole „kompetencją, na której student wypadł dobrze", w drugim kształcie byłby to **Python — moduł PŁYTSZY niż dziura** — a zdanie „a w tej ścieżce to fundament pod resztę" twierdziłoby wtedy, że Pandas jest fundamentem pod Pythona. Nieprawda o własnej drabinie, w zdaniu, które ma tłumaczyć. Nowy tekst mówi to samo, opierając się wyłącznie na module, który **z definicji** istnieje: samej dziurze.

  **Zdanie, które zostało utrzymane świadomie:** „To wynik dwóch krótkich pytań, nie ocena Ciebie" — to jedyny powód dziury, przy którym student **realnie odpowiadał** na pytania z tej kompetencji i realnie wypadł poniżej progu (§12.4 reguła 1). Cisza w tym jednym miejscu byłaby ciszą wobec jedynej osoby, która ma czym ją sprawdzić.
...
### [NOWE v0.10] Reguła interpolacji — rozszerzona na WSZYSTKIE teksty o dziurze

Wersja z v0.8 (niżej) ograniczała się do sesji **z zerem odblokowań** — i dlatego nie złapała błędu, który Jack znalazł przy niezerowych. Brzmienie wiążące od v0.10:

**Tekst o dziurze wolno zbudować wyłącznie z dwóch źródeł: (a) własnych pól dziury — nazwa kompetencji i tytuł modułu; (b) tytułu ostatniego odblokowanego modułu, i to tylko wtedy, gdy odblokowania są niepuste. Żadnego trzeciego modułu ani żadnej trzeciej kompetencji.**

**Kategoria „kompetencja głębsza" jest zamknięta na stałe**, nie odłożona. Uzasadnienie jest własnością reguły placementu, nie tej wersji dokumentu: dziurą zostaje **pierwszy** otagowany moduł, który nie spełnia warunku, więc reguła **nie gwarantuje istnienia** żadnego zakwalifikowanego modułu głębiej — ani, gdy taki istnieje, tego, że jest głębszy od dziury w sensie, którego wymaga zdanie. Każdy tekst oparty na takim polu będzie milczał w części sesji albo kłamał w innej. Dotyczy to tak samo pól „kompetencja płytsza", „pierwszy nieodblokowany" i każdego innego modułu spoza pary (dziura, ostatni odblokowany).

**Konsekwencja dla kontraktu danych:** §12.8 pkt 3 zostaje przy **trzech** polach dziury i **nie wolno go rozszerzać** o czwarte na potrzeby mikrocopy. Jeśli kiedyś zdanie będzie wymagać czwartego pola — to znak, że złe jest zdanie.

**REGUŁA GENERALNA (v0.8, nadal obowiązuje jako przypadek szczególny) — wiążąca dla każdego przyszłego tekstu o dziurze:** przy zerze odblokowań wolno interpolować **wyłącznie własne pola dziury** — nazwę kompetencji z dziury i tytuł modułu z dziurą. **Nigdy** „ostatniego odblokowanego" (nie istnieje) i **nigdy** „kompetencji głębszej" (też nie musi istnieć: przy `ds-python`=2 i reszcie niezaznaczonej nie ma żadnego wysokiego wyniku, do którego dałoby się nawiązać). Bez tej reguły ta sama luka wraca przy każdym nowym powodzie — dopisuję ją jako regułę, a nie jako dwa teksty, bo dwa teksty łatają objaw.
...
**[NOWE v0.9] Dwie powierzchnie to liczba zamknięta — trzeciej lokalizacji w L6 NIE MA.** Mila słusznie zgłosiła, że §12.4 (v0.8) przypisywał powodom `qualified` i `carried_untagged` „kartę wejścia w moduł" na stronie `/curriculum/[moduleId]` — czyli **trzecie** miejsce, którego ta tabela nie wymieniała. To była sprzeczność wewnętrzna mojego dokumentu, nie luka w jej projekcie: mechanizm z §12.4 opierał się na powierzchni, której mój własny zakres nie deklarował (dokładnie ten błąd, który brama A2 ma wyłapywać). **Rozstrzygnięcie: zawężam §12.4 do powierzchni B, karta wejścia zostaje poza L6** — uzasadnienie merytoryczne w nocie pod tabelą 12.4, zaksięgowanie z progiem i właścicielem w §13 / **D12**. `src/app/(dashboard)/curriculum/[moduleId]/page.tsx` **nie jest dotykany w L6**.
...
3. NADPISANIE (DECYZJA 3): jeśli moduł pozycji 1 (`l0-start`) nie ma statusu
   'completed' ORAZ JEST KANDYDATEM (krok 1) → rekomendacja := `l0-start`,
   niezależnie od kroku 2                                    [doprecyzowane v0.10]
4. brak kandydatów → brak rekomendacji (wariant 9 z 12.6)
```

**[NOWE v0.10] Doprecyzowanie kroku 3 — ratyfikacja interpretacji Jacka.** Jack zapytał w komentarzu (`placement-screen.ts`, `pickRecommendation`), czy „niezależnie od kroku 2" znaczy także „niezależnie od tego, czy korzeń jest kandydatem". **Nie znaczy — nadpisanie działa tylko wtedy, gdy korzeń jest kandydatem.** Korzeń w stanie `coming_soon` (moduł bez pozycji) nie może zostać zarekomendowany z tego samego powodu, dla którego wykluczyłam `coming_soon` w v0.8: rekomendacja ma wskazywać moduł, w którym **da się dziś coś zrobić**. Literalne czytanie dawałoby zdanie „Zacznij od Start: środowisko pracy" o module, w którym nie ma ani jednej pozycji — czyli rekomendację w pustkę, i to w jedynym zdaniu, po którym student ma coś zrobić.

**Konsekwencja, którą świadomie przyjmuję:** przy korzeniu bez treści rekomendacja spada na najgłębszego kandydata, więc student może dostać wskazanie modułu **bez uprzedniego ustawienia środowiska** — czyli scenariusz, przed którym chroni DECYZJA 3. Wybieram to jako mniejsze zło (wskazanie działającego modułu kontra wskazanie pustego) i nazywam, zamiast przemilczeć: `l0-start` ma cztery pozycje z mocy D10, więc stan „korzeń `coming_soon`" oznacza **błąd ingestu**, nie normalną pracę. Jeśli kiedykolwiek wystąpi, właściwą naprawą jest ingest, nie rekomendacja.
...
**[NOWE v0.9] Nagłówek sekcji: „Po diagnozie" — tekst wiążący, źródło w §8.** Mila zaproponowała „Twoja ścieżka" i zgłosiła to zamiast wstawić po cichu (słusznie — reguła „teksty 1:1 z §8" jest twarda). **Odrzucam ten nagłówek, z trzech powodów, każdy wystarczający:** (1) `/curriculum` nazywa się „Ścieżka nauki" — sekcja o tej samej nazwie pokazuje **węższą** treść niż strona, do której odsyła, czyli produkuje dokładnie ten rozjazd, przed którym broni §12.2 („ekran mówiący co innego niż drabina"); (2) nagłówek ramuje listę pod sobą, a blok 2 jest listą modułów — „Twoja ścieżka" nad listą czterech modułów podaje **wycinek jako stan ścieżki**, wprost zakazany w §12.7 pkt 1; (3) w tym samym pliku stoi już sekcja „Twój plan nauki" — dwie bliźniacze etykiety dzierżawcze o różnych treściach na jednym ekranie. Rodzina „punkt startu" jest zamknięta osobno (§7: żadnej z trzech wielkości nie wolno tak nazwać).

**Wybrane brzmienie: „Po diagnozie".** Nie twierdzi nic o stanie ścieżki, nie zawiera liczby ani listy, nie podporządkowuje pracy studenta naszej diagnozie (§12.10 — parytet bloku 1 i 2) i jest prawdziwe we **wszystkich** wariantach renderu, w których sekcja w ogóle powstaje. Znacznik jest **czasowy** (moment czytania), nie własnościowy — dlatego blok 1 („Masz już zaliczone…") stoi pod nim bez fałszu: nagłówek nazywa chwilę, pierwsze zdanie nazywa właściciela dorobku.

**[NOWE v0.9] Reguła generalna — etykiety strukturalne tego ekranu też są mikrocopy.** Nagłówki sekcji, etykiety `aria-label` i każdy inny tekst widoczny albo odczytywany przez czytnik ekranu w obrębie sekcji placementu podlegają §8 i §12.7 tak samo jak zdania narracji. Powód: nagłówek **ramuje** zdania pod sobą, a ramowanie jest treścią — „Twoja ścieżka" nad poprawnym zdaniem „Diagnoza otworzyła dodatkowo…" zamienia poprawne zdanie w twierdzenie o stanie. Dopisuję to jako regułę, a nie jako jeden zatwierdzony nagłówek, bo inaczej ta sama luka wróci przy każdej kolejnej sekcji. Teksty w §8.

**[NOWE v0.9] Blok 3 zostaje TEKSTEM — nie linkiem i nie przyciskiem.** Rozstrzygnięcie na pytanie 3 Mili. **Najpierw prostuję fakt, bo jej uzasadnienie jest nieprawdziwe i zostawione bez korekty doprowadziłoby do złej naprawy:** `POST /api/onboarding` (jedyny zapis `students.career_goal`) leci **wewnątrz `handleDiagnosisFinished`**, przed przejściem na krok 4 (`onboarding-wizard.tsx:429` → `runSubmit` → `advanceTo(4)`). W chwili, gdy student widzi ten ekran, cel kariery jest **już w wierszu studenta**, więc `/curriculum` rozpoznaje ścieżkę, a link **nie byłby martwy**. Przycisk „Przejdź do pulpitu" woła co innego — `POST /api/onboarding/complete`, czyli **domknięcie** onboardingu (`onboardingCompleted=true`), nie zapis profilu. Naprawa D0 tego obrazu nie zmienia w żadną stronę: D0 dotyczy źródła celu dla **hooka placementu** w chwili domknięcia diagnozy, a nie tego, co widzi `/curriculum` **po** zapisie profilu.

**Powody produktowe, dla których mimo to nie linkuję** (trzy, w kolejności wagi):

1. **Link ominąłby powierzchnię B przy pierwszym kontakcie.** Powierzchnia A znika po jednym przejściu; jedynym trwałym nośnikiem zdania „otwarte ≠ zaliczone" jest drabina z odznaką (§12.7 pkt 6). Wysłanie studenta prosto do środka modułu przy pierwszym zetknięciu z placementem omija dokładnie ten ekran, który uznałam za nieopcjonalny.
2. **Czwarty przycisk nie wzmacnia rekomendacji — rozpuszcza ją.** Krok „Wnioski" ma już trzy wyjścia (`onComplete()` na pulpit, `onComplete("/gap-analysis")`, `onComplete("/projects")` — `step-wnioski.tsx:123,338,346`). §12.10 przyznaje blokowi 3 status **jedynego akcentu**; czwarty przycisk w rzędzie równorzędnych zamienia „jedyne zdanie, po którym student ma coś zrobić" w jedną z czterech ofert.
3. **Rekomendacja w pilotażu to prawie zawsze `l0-start`** (nadpisanie z DECYZJI 3), czyli **pierwszy wiersz drabiny**. Koszt „szukania modułu samodzielnie", który link miał usunąć, wynosi tu jedno spojrzenie na górę listy — nie uzasadnia oddania akcentu i ominięcia powierzchni B.

**[WYMÓG — Jack, wiążący, na wypadek przyszłej zmiany]** Jeśli rekomendacja kiedykolwiek stanie się klikalna, to **nigdy** surowym `<Link>`/`<a>` z kroku 4: wyjście z kreatora poza `onComplete()` zostawia `onboardingCompleted=false`, więc powrót do `/onboarding` **wznawia kreator**, który student uważa za skończony. Jedyna dopuszczalna forma to istniejący wzorzec `onComplete(target)`, a celem ma być **`/curriculum`** (drabina z odznaką), nie `/curriculum/[moduleId]`. Zmiana wymaga mojego sign-offu, bo dotyka akcentu z §12.10.
...
|---|---|---|---|
| `qualified` | moduł | **[v0.9] wyłącznie odznaka na drabinie (powierzchnia B)** | „Otwarty na podstawie diagnozy · niezaliczony". Zdanie o trybie wsparcia (§8, moduł graniczny) **nie ma w L6 powierzchni** — nota niżej, §13 / D12 |
| `carried_untagged` | moduł | **[v0.9] wyłącznie odznaka na drabinie (powierzchnia B)** | odznaka jak wyżej. Zdanie o otwarciu cudzym pomiarem (§8, F2/F3) **nie ma w L6 powierzchni** — nota niżej, §13 / D12 |
...
### [NOWE v0.9] Dlaczego karta wejścia w moduł wypada z L6 — rozstrzygnięcie

**Rozstrzygnięcie jednym zdaniem: karta wejścia w moduł NIE wchodzi do L6 przed zapłonem, bo komunikat, który miała nieść, opisuje decyzję bez skutku — a produkt nie mówi studentowi o mechanizmach, których nie ma.**

**Fakt, na którym to stoi (odczytany z kodu, nie z pamięci): `supportMode` nie ma dziś ani jednego czytelnika.** Pisarze: `computePlacement` wylicza wartość (`src/lib/curriculum/placement.ts:422`, funkcja `supportMode` w linii 459), serwis zapisuje ją do kolumny (`placement-service.ts:411`), schemat ogranicza do `('full','fading')` (`schema.ts:2237-2260`), testy ją asertują. **Poza tym łańcuchem nie sięga po nią nic** — ani widok modułu (`src/app/(dashboard)/curriculum/[moduleId]/page.tsx`), ani odtwarzacz pozycji, ani drabinka podpowiedzi. Drabinka podpowiedzi (ADR-018, `src/lib/curriculum/hints.ts`) jest **odsłaniana na żądanie studenta** i ma ten sam sufit trzech poziomów dla każdego, niezależnie od trybu wsparcia.

**Skutek dla treści karty.** Zdanie „zostawiamy pełne wsparcie włączone" jest dziś literalnie prawdziwe (wsparcie jest pełne dla wszystkich) i **produktowo nieprawdziwe w implikaturze**: sugeruje decyzję podjętą o tym studencie i różnicę wobec innych, a żadnej różnicy nie ma. Student, który wyklika `test out` z innego modułu i porówna widoki, zobaczy identyczny ekran. To ta sama klasa błędu, którą sama nazwałam w D7 („bez skrótów" nad odznaką „Otwarty na podstawie diagnozy") — z tą różnicą, że tam odziedziczyłam cudze zdanie, a tutaj napisałabym własne, wiedząc.

**Dlaczego to NIE osłabia trzeciej gałęzi trybu wsparcia z DECYZJI 2.** Rozdzieliłam `carried_untagged` od poziomu 3 z powodu **danych, nie komunikatu** — cytat z DECYZJI 2: „powód musi zostać w danych rozdzielony, inaczej miernik przestaje odróżniać dwie różne decyzje, które wymagają przeciwnych napraw". Ta gałąź żyje w bazie, bo tam ma żyć: jej odbiorcą jest miernik (D11), nie student. Argument „bez karty tryb wsparcia nie ma ani jednej powierzchni" jest prawdziwy i **nie jest zarzutem** — nadanie powierzchni rozróżnieniu, które nic nie zmienia w doświadczeniu, byłoby obietnicą mechanizmu.

**Prostuję własną przesłankę, żeby decyzja nie stała na fałszu.** Argument „przy suficie D1 populacja, która zobaczy moduł otwarty placementem, może być bliska zeru" **jest nieprawdziwy**. `f1-python-1` (pozycja 2) kwalifikuje się na samym `ds-python ≥ 3`, czyli na **jednym** trafionym pytaniu trudności 2, a Python jest pozycją 1 katalogu rynku DS (55% ofert) — zaznaczy go niemal każdy. `qualified` na F1 jest więc **najbardziej prawdopodobnym niezerowym wynikiem pilotażu**, nie przypadkiem brzegowym; sufit D1 obcina `m-sql`/`m-ml`/`m-llm`, nie fundament. Odkładam kartę **mimo** istniejącej populacji — bo nie ma jej czego zakomunikować, a nie dlatego, że nie ma komu.

**Co student traci bez karty: prawie nic.** Pochodzenie dostępności niesie odznaka na powierzchni B, o jedno kliknięcie wcześniej, w miejscu, do którego wraca przez cały pilotaż (§12.7 pkt 6). Karta dokładałaby wyłącznie **przesłankę** otwarcia („jedno trafione pytanie" / „bo poradziłeś sobie z X") — informację prawdziwą, ale bez konsekwencji dla tego, co student ma zrobić.

**Teksty z §8 dla obu powodów zostają nietknięte** jako gotowe wejście dla D12. Nie kasuję ich: są poprawne, a ich napisanie było pracą dydaktyczną, nie techniczną. Zmienia się **wyłącznie** to, że dziś nie mają powierzchni — i jest to zapisane, a nie przemilczane.

**[WYMÓG — Jack, wiążący]** W L6 nie powstaje żaden komponent na `/curriculum/[moduleId]`. Powody `qualified` i `carried_untagged` mają w L6 dokładnie jedno wyjście: odznakę z §12.9 pkt 2.
...
**[NOWE v0.10] Trzy uzupełnienia z implementacji Jacka — ratyfikowane, przeniesione tu z kodu:**

1. **Warianty 1–4 („sekcja nie istnieje") realizuje BRAK KLUCZA w odpowiedzi trasy, nie wartość `null`.** To jest dosłowne wykonanie mojego zdania „sekcja nie istnieje", nie jego interpretacja — i różnica ma znaczenie produktowe, nie tylko techniczne: klucz o wartości `null` jest **informacją, że placement się liczył i nic nie dał**, a tego akurat wariant 1 (flaga OFF) i wariant 2 (cel spoza pilotażu) **nie stwierdzają**. Brak klucza nie twierdzi nic — i o to chodziło.
2. **Wariant 8 × wariant 10 (zaliczenia > 0, odblokowań 0, brak modułu do polecenia).** Zdanie wariantu 8 kończy się rekomendacją („…zacznij od **{moduł}**"), więc bez modułu **nie da się go dokończyć**. Render: blok 1 + zdanie zastępcze z wiersza 10 („Masz zaliczone wszystko, co dziś jest w ścieżce." albo „Kolejny moduł jest w przygotowaniu."). Jack nie zbudował hybrydy dwóch zdań i miał rację — hybryda byłaby nowym tekstem, a nowych tekstów nie pisze wykonawca.
3. **Wariant 10 rozdzielony na dwa stany w kontrakcie** (`all_completed` kontra `only_coming_soon`), bo niosą **dwa różne zdania** i dwie różne odpowiedzialności: „masz wszystko" to sukces studenta, „treść w drodze" to **nasza** zaległość. Zlanie ich w jedno pole zmusiłoby UI do zgadywania, które zdanie powiedzieć.
...
**[NOWE v0.9] Jednorazowość jest mocniejsza, niż zapisałam w v0.8 — student może nie zobaczyć tego ekranu ANI RAZU.** Wynik diagnozy żyje w stanie komponentu kreatora (`diagnosisOutcome`), a domknięcie onboardingu (`onboardingCompleted`) zapala się dopiero przyciskiem „Przejdź do pulpitu". Student, który po diagnozie odświeży stronę albo zamknie kartę przed tym kliknięciem, wraca do wznowionego kreatora **bez** wyniku w stanie — czyli bez sekcji placementu. Nie jest to defekt do naprawy (naprawa oznaczałaby utrwalanie powodów, czyli złamanie minimalizacji z L3 i niezmiennika „zapis w chwili odblokowania"), tylko **konsekwencja wzmacniająca powierzchnię B**: drabina z odznaką jest jedynym miejscem, o którym wiemy, że student je zobaczy. Zapisuję to, żeby nikt nie planował komunikacji „otwarte ≠ zaliczone" wyłącznie na powierzchni A.
...
**[NOWE v0.10] Trzy uzupełnienia kontraktu — ratyfikowane po implementacji:**

7. **`deepestUnlockedTitle` wypełniamy ZAWSZE, gdy coś się otworzyło** — nie tylko przy zerze zaliczeń. Kontrakt propsów Mili zawężał to pole do wariantu „brak zaliczeń", ale tekst `no_measurement` przy niezerowych odblokowaniach też go interpoluje, a od v0.10 robi to również przepisany tekst `below_threshold`. **To jest fakt o sesji („dokąd otworzyliśmy"), nie tryb renderu** — a pole, którego wypełnienie zależy od tego, który wariant akurat się renderuje, jest pułapką dla następnego tekstu.
8. **Kontrakt niesie, czy rekomendacja jest KORZENIEM drabiny** (pole logiczne, nie porównanie sluga). Ekran nie ma prawa tego wyprowadzić: „pozycja 1" jest własnością drabiny, a wyprowadzanie w UI („brak zaliczeń ⇒ to korzeń") byłoby drugą implementacją reguły z §12.2, czyli złamaniem punktu 1 wyżej. Wariant „porównaj slug z `l0-start`" wpiekłby slug w UI (zakaz §12.7 pkt 4). Pole dołożone przez Jacka, addytywnie — przyjmuję.
9. **Dziura zostaje przy TRZECH polach i nie wolno dokładać czwartego** dla potrzeb mikrocopy — uzasadnienie i konsekwencja w §8, „Reguła interpolacji".
...
**[NOWE v0.9] Trzy poprawki do projektu Mili** (`docs/design/1e7-l6-ekran-wyniku-diagnozy-v0.1.md`) — wynikają z rozstrzygnięć v0.9, nie z zastrzeżeń do warstwy wizualnej, którą przyjmuję w całości (hierarchia, brak ikon, reużyte tokeny, kształt odznaki):

1. **Nagłówek „Twoja ścieżka" → „Po diagnozie"** (§3.7 i §5.3 jej dokumentu, `<h3>` w `placement-summary.tsx`). Uzasadnienie: §12.3.
2. **`aria-label` sekcji → „Po diagnozie"**, zamiast „Twoja ścieżka po diagnozie" (§5.3 jej dokumentu). Dwa powody: zawiera odrzucone słowo w odrzuconym znaczeniu, a poza tym czytnik ekranu dostawałby frazę, której widzący student nigdy nie zobaczy — dwie wersje tej samej etykiety to dwie wersje prawdy.
3. **Jej §9 pkt 2 i §12 („brak linku, bo `/curriculum` czyta cel z wiersza studenta") wymaga sprostowania faktu** — cel jest w wierszu **przed** renderem kroku 4 (§12.3). Decyzja „bez linku" **zostaje**, ale na innym uzasadnieniu; uzasadnienie oparte na nieprawdziwym fakcie upadnie przy pierwszym czytaniu kodu i pociągnie za sobą decyzję, która jest słuszna.

**[NOWE v0.10] Czwarta poprawka — etykieta czytnika ekranu na kontenerze odznak (jej §6) ODRZUCONA.** Propozycja brzmiała: `aria-label` na rzędzie odznak, żeby czytnik przeczytał status i odznakę placementu jako jedno zdanie. Jack wykazał, że kontener leży **wewnątrz linku wiersza**, więc etykieta na potomku **podmienia nazwę całego linku** — wszystkie wiersze drabiny brzmiałyby dla czytnika ekranu identycznie, **bez tytułu modułu**. To nie jest kompromis dostępnościowy, tylko strata informacji, i to dokładnie tej, która identyfikuje moduł. **Rozstrzygnięcie: bez etykiety.** Odznaka jest pełnym zdaniem („Otwarty na podstawie diagnozy · niezaliczony"), więc czytnik odczyta ją jako tekst — warunek z §12.10 („czytelna tekstem, nie samym kolorem") jest spełniony bez żadnego `aria-*`. Decyzja techniczna Jacka, ale skutek produktowy jest mój i dlatego zapisuję go tutaj, a nie zostawiam w komentarzu w kodzie.

**Bez zmian przyjmuję jej §3.4** (wariant 8 bez osobnego bloku 3 — zdanie z §8 już niesie rekomendację; powielenie akcentu byłoby dwiema rekomendacjami w dwóch stylach) **i §4.3** (podwójny guard odznaki: `openedByPlacement && status !== 'completed'` — wiersz placementu jest z mojej decyzji nienadpisywany i nigdy nie znika, więc front ma prawo nie ufać mu w pytaniu „czy TERAZ pokazać").

---

## 13. Rejestr długu produktowego 1E.7 [NOWE v0.8]

Inwentaryzacja na zlecenie Olivera („nie zostawiaj za sobą żadnych długów"). Waga = koszt niezrobienia. Próg = najpóźniejszy moment, w którym pozycja musi być zamknięta. **Osiem pozycji dopisanych ponad listę Olivera** (D0, D7–D13). [v0.9: 13 pozycji — dołożona **D12** (karta wejścia w moduł), doprecyzowany próg D5 i D11. **v0.10: 14 pozycji — dołożona D13** (cisza wyjaśnienia w wariancie 8).]
...
| **D13** | wyjaśnienie dziury milczy u studenta z zaliczeniami i zerem odblokowań (wariant 8) | niska (dziś praktycznie nieosiągalna) | mały (trzy zdania) | **przed pierwszym re-onboardingiem studenta z zaliczonym modułem** | Sophia |
| **D12** | karta wejścia w moduł — tryb wsparcia bez powierzchni **i bez mechanizmu** | niska dziś, wysoka przy wygaszaniu wsparcia | mały (teksty gotowe) + nieznany (mechanizm nie istnieje) | **razem z pierwszym czytelnikiem `supportMode`** | Sophia (tekst, umiejscowienie) + Ethan (mechanizm) |
| **D5** | zakres wniosków z pilotażu bez nośnika poza tym dokumentem | średnia | mały | **przed pierwszą diagnozą 1. uczestnika** | Sophia + Oliver |
| **D11** | zdarzenie miernika nie ma odbiorcy (brak zapytania odczytującego) | średnia-wysoka | mały | **przed pierwszą diagnozą 1. uczestnika** | Ethan + Ryan |
...
### D13 — cisza wyjaśnienia u studenta z zaliczeniami i zerem odblokowań [NOWE v0.10]

**Stan faktyczny (Jack, przypięty testem).** Wszystkie trzy warianty zerowe z §8 mówią „**zaczynamy od początku ścieżki**". U studenta, który ma **zaliczone moduły**, to zdanie jest nieprawdą — nie zaczyna od początku, ma dorobek, o którym blok 1 właśnie powiedział. Jack wybrał ciszę zamiast nieprawdy i **to jest właściwy wybór wobec moich reguł**: nieprawda w zdaniu, którego jedynym zadaniem jest tłumaczyć, jest gorsza niż brak zdania.

**Dlaczego NIE naprawiam tego teraz, skoro naprawiam bliźniaczą ciszę przy `below_threshold`.** Bo te dwa przypadki mają różną osiągalność, a nie różną wagę. Cisza przy `below_threshold` z odblokowaniami trafia w studenta, który **odpowiadał na pytania i wypadł poniżej progu** — w pilotażu osiągalne przy pierwszym lepszym „Python OK, Pandas słabo". Wariant 8 wymaga łącznie: zaliczonego modułu **i** ponownej diagnozy (a te są wyłącznie przez re-onboarding, §6b) **i** dziury **i** zera nowych odblokowań. Przy 1–3 studentach profilu „literalne zero" to nie jest przypadek, który zobaczymy — a każdy tekst dopisany dziś byłby pisany bez ani jednej obserwacji.

**Kształt naprawy, żeby nie trzeba było jej wymyślać od nowa:** trzeci wariant framingu, **bez** zdania o tym, gdzie zaczynamy. Wzorem jest istniejący tekst `uncovered` przy niezerowych odblokowaniach, który o starcie nie mówi nic: „…więc moduł **{tytuł}** zostaje na swoim miejscu. Jeśli znasz ten materiał — zdaj egzamin modułu (**test out**) i przeskocz go." Ta konstrukcja jest prawdziwa **niezależnie** od dorobku studenta, więc trzy teksty wariantu 8 powstają przez podmianę zdania o starcie, nie przez pisanie od zera.

**Próg:** przed pierwszym re-onboardingiem studenta, który ma zaliczony moduł — czyli w praktyce przy pierwszej funkcji „zmień kierunek" używanej przez kogoś z dorobkiem. **Właściciel: ja** (to mikrocopy, nie kontrakt — kontrakt niesie już wszystkie potrzebne pola).

### D12 — karta wejścia w moduł: tryb wsparcia bez powierzchni i bez mechanizmu [NOWE v0.9]

**Co zostało odłożone.** Zdanie przy wejściu w moduł otwarty placementem — dla `qualified` przy `supportMode='full'` („na podstawie jednego trafionego pytania… zostawiamy pełne wsparcie włączone") i dla `carried_untagged` („tego modułu nie sprawdzaliśmy w diagnozie… bo poradziłeś sobie z X"). Teksty są **gotowe i niezmienione w §8**; odłożona jest wyłącznie **powierzchnia**, czyli render na `/curriculum/[moduleId]`. Pełne uzasadnienie: nota pod tabelą §12.4.

**Dlaczego to nie jest odłożenie z braku czasu.** `supportMode` nie ma czytelnika: wartość jest liczona, zapisana i ograniczona w schemacie, ale **nic w produkcie nie zachowuje się inaczej** w zależności od niej. Karta mówiłaby studentowi o decyzji bez skutku. Zapalenie jej przed mechanizmem oznaczałoby świadome wyprodukowanie drugiego D7 — zdania, któremu przeczy kod pod nim.

**Próg — przypięty do zdarzenia, nie do fazy:** **pierwszy czytelnik `supportMode`**, czyli pierwsza funkcja, która realnie różnicuje przejście modułu według trybu wsparcia (wygaszanie drabinki podpowiedzi, wejście od fazy completion — C7/C8 z D8). **Nigdy wcześniej.** Odwrotna kolejność (karta przed mechanizmem) jest tu wprost zabroniona, nie tylko niezalecana.

**Warunek domknięcia (dla mnie, w chwili gdy próg zadziała):** zanim karta powstanie, muszę zdecydować, czy zdanie o wsparciu opisuje wtedy **fakt o produkcie** („masz włączone wszystkie podpowiedzi, bo…"), czy nadal tylko zapis w bazie. Jeśli mechanizm różnicuje coś, czego student nie może zobaczyć, karta zostaje odłożona dalej — reguła jest o **sprawdzalności zdania**, nie o istnieniu kolumny.

**Warunek samoczyszczący (żeby ta pozycja nie żyła wiecznie).** Próg D12 jest przypięty do zdarzenia, które może **nigdy** nie nastąpić — wygaszanie wsparcia nie jest dziś w żadnym planie. Dlatego drugi, bezwarunkowy termin: **przy pierwszej rewizji §8** (a ta przyjdzie z D1/D2, rewizją banku pytań) oba teksty kart albo dostają powierzchnię, albo **znikają z §8**. Mikrocopy bez powierzchni wolno trzymać przez jedną rewizję jako gotowe wejście; dłużej staje się treścią, którą kolejny wykonawca uzna za obowiązującą i wstawi gdziekolwiek, bo „przecież jest w §8".

**Waga dziś niska, ale nie zerowa:** bez karty przesłanka otwarcia (`jedno trafione pytanie` / `cudzy pomiar`) nie ma gdzie wybrzmieć, więc student z otwartym `f2-python-2` nie dowie się, że otworzył go pomiar z `m-pandas`. Odznaka mówi „otwarty na podstawie diagnozy" — prawdę, tylko mniej precyzyjną. To akceptowalna strata; nieakceptowalna byłaby precyzja kupiona zdaniem o nieistniejącym mechanizmie.
...
**[v0.9] Stan nośnika (a): w drodze.** Dokument jest zacommitowany na gałęzi `feat/1e7-l6-domkniecie` (`5a00082`); scalenie prowadzi Oliver z Ethanem. Pozycja zostaje otwarta do chwili scalenia — commit na gałęzi roboczej nie jest jeszcze istnieniem dla czytelnika, który tej gałęzi nie zna.

**[v0.9] Odpowiedź na pytanie Olivera: próg D5(b) i D11 zostaje na „przed 1. studentem" — z doprecyzowanym momentem, nie z przesunięciem.** Po L4 i L5 nie zmieniło się nic, co uzasadniałoby przeniesienie któregokolwiek przed zapłon: zapłon to zapalenie flagi, a flaga bez studenta **nie produkuje ani jednego zdarzenia**, więc zapytanie czytające pusty dziennik nie ma czego przekłamać, a ograniczenie wnioskowania nie ma nikogo przed czym chronić. Postawienie ich jako bramy zapłonu byłoby blokowaniem wdrożenia dostawą, która do dnia pierwszej diagnozy pozostaje bezczynna — i tego rodzaju bramka umiera na własnej bezużyteczności, a razem z nią dostawa. **Doprecyzowuję jednak moment, bo „przed 1. studentem" to faza, a fazy nie mają daty:** próg brzmi **„przed pierwszą diagnozą pierwszego uczestnika pilotażu"** — czyli zamknięte, zanim powstanie pierwsze zdarzenie, nie „gdzieś w trakcie pilotażu". Ryzyko materializuje się na **pierwszym** punkcie danych, nie na dziesiątym: jedna sesja z `unlockedCount: 0` wystarczy, żeby ktoś przeczytał ją jako „próg za wysoki", a przy `blockingHoleReason = no_measurement` próg nie miał nic do roboty. Jedno się natomiast **dołożyło po v0.8**: D0 wprowadza nowy stan do odczytania — „cel z sesji pusty albo nierozwiązywalny na ścieżkę" ma zostawiać ślad w dzienniku, więc zapytanie z D11 musi go objąć i odróżnić od „student ma cel spoza pilotażu DS". Bez tego pierwsza cisza w danych będzie nieodróżnialna od defektu, który właśnie naprawiamy.
...
Wymóg minimalny (przy 1–3 studentach to praca ręczna, ale ręczna praca bez zapisanego zapytania nie zostanie wykonana): jedno zapisane zapytanie po `action = 'curriculum.placement.computed'`, zwracające `unlockedCount`, `blockingHoleSlug`, `blockingHoleReason`, `blockingHoleLevel`, `alreadyCompletedCount`, `threshold` — z **ograniczeniami wnioskowania obok** (D5b). **[NOWE v0.11 — WYMÓG TWARDY] Zapytanie miernika MUSI wykluczać przebiegi weryfikacyjne, a pierwszy wiersz na produkcji JEST takim przebiegiem.** 1E.7 zapalone 2026-08-01; pierwsze zdarzenie i pierwszy wiersz `curriculum_placements` pochodzą z weryfikacji zapłonu kontem QA (Ethan), nie od uczestnika. Wygląda to jak **udany** placement (`qualified`, poziom 4, jedno odblokowanie, dziura `no_measurement`), więc bez wykluczenia pierwszy odczyt danych pokaże stuprocentową skuteczność zbudowaną na odegranej sesji. **Sposób wykluczenia jest regułą, nie filtrem po polu:** zdarzenie przypisuje się do sesji diagnozy **nazwanego uczestnika** albo nie jest obserwacją (pełne uzasadnienie i dowód z kodu — §6a, „Pierwszy wiersz na produkcji jest TECHNICZNY"). **Nie wolno oprzeć tego wykluczenia na atrybutach konta** (domena `.invalid`, oznaczenie uczelni): zdarzenie nie ma `actorId`, więc do konta dociera się wyłącznie złączeniem przez `assessment_sessions`, a to złączenie znika kaskadą przy skasowaniu konta — filtr przestałby działać dokładnie wtedy, gdy jest potrzebny.

**[v0.9] Zakres rozszerzony przez D0:** zapytanie musi objąć także ślad „cel sesji pusty albo nierozwiązywalny na ścieżkę" (wymóg kontrolny z D0) i pokazywać go **osobno** od `unlockedCount: 0` — pierwszy to defekt liczenia, drugi to poprawny wynik dla persony pilotażu. Zlanie ich w jedną ciszę kasuje sens tego miernika w jedynym pilotażu, jaki mamy. **Mianownik bierzemy z listy uczestników pilotażu, nie z dziennika**: student, który nie zaznaczy żadnej kompetencji, nie odbywa testu, więc nie zostawia zdarzenia (§6a pkt 5, jedyna jawna dziura w pomiarze).
...
| **11. [v0.9, zlecenie Olivera] Rozstrzygnięcie trzech pytań Mili do §12** (karta wejścia w moduł: wchodzi czy nie; nagłówek sekcji na powierzchni A; blok 3 tekstem czy linkiem) | **§12.4** (nota „Dlaczego karta wejścia w moduł wypada z L6") · **§12.3** (nagłówek + blok 3) · **§8** (nagłówek jako tekst wiążący) · **§12.10** (trzy poprawki do projektu Mili) | ✔ |
| **12. [v0.9, zlecenie Olivera] Odpowiedź o progach D5(b) i D11 po L4/L5** (przed zapłonem czy przed 1. studentem) | **§13 / D5** (akapit „Odpowiedź na pytanie Olivera") + **§13 / D11** (rozszerzenie zakresu przez D0) + tabela §13 (próg doprecyzowany) | ✔ |

| **13. [v0.10, zlecenie Olivera] Rozstrzygnięcie luki zgłoszonej przez Jacka** (cisza przy `below_threshold` z niezerowymi odblokowaniami: A/B/C) | **§8** (przepisany tekst + „Reguła interpolacji") | ✔ |
| **14. [v0.10, zlecenie Olivera] Przeniesienie sześciu rozstrzygnięć Jacka z kodu do dokumentu** | **§8** (tekst korzenia) · **§12.6** (trzy uzupełnienia) · **§12.8** (pkt 7–9) · **§12.10** (etykieta czytnika ekranu) · **§13 / D13** (cisza w wariancie 8) | ✔ |
| **15. [v0.10, spoza listy] Rozstrzygnięcie interpretacji kroku 3 algorytmu rekomendacji** (prośba Jacka w komentarzu `placement-screen.ts`) | **§12.2** (doprecyzowanie + nazwana konsekwencja) | ✔ |

| **16. [v0.11, zlecenie Olivera] Nośnik faktu o technicznym pierwszym wierszu na produkcji** (co to jest, po czym odróżnić, wykluczenie z odczytu miernika) | **§6a** („Pierwszy wiersz na produkcji jest TECHNICZNY" — reguła odczytu + co przebieg dowiódł, a czego nie) + **§13 / D11** (wymóg dla zapytania) + „Stan realizacji" (zapłon) | ✔ |

Żadne dwie dostawy nie wskazują tej samej sekcji. **[v0.11]** Dostawa 16 ma główny nośnik w §6a (ograniczenia wnioskowania — tam czyta czytelnik danych) i **wskazanie** w D11, gdzie powstanie zapytanie; podział ról bez zmian: §6a niesie **regułę**, §13 **wymóg wobec przyszłej pracy**. **[v0.10]** Dostawy 13–15 wnoszą **poprawki i ratyfikacje** do sekcji dostaw 9 i 11; rozdział ról bez zmian — §8/§12 to treść wiążąca, §13 to rejestr z progiem. Jedyna pozycja padająca w obu (D13) jest w §12.6/§8 opisana jako **zachowanie renderu**, a w §13 wyłącznie **zaksięgowana z progiem i właścicielem**. **[v0.9]** Dostawa 11 dotyka §12.3/§12.4/§8/§12.10, czyli sekcji należących do dostawy 9 — ale wnosi do nich **rozstrzygnięcia**, a nie powtórzenie specyfikacji: dostawa 9 to specyfikacja ekranu, dostawa 11 to trzy werdykty, które ją **zawężają** (jedna powierzchnia mniej, jeden nagłówek zmieniony, jedna forma bloku 3 potwierdzona). Dostawa 12 ma własny nośnik w §13 i nie powiela §12. D12 pada w §12.4 (jako wymóg wykonawczy: „nie budujemy") i w §13 (jako pozycja z progiem i właścicielem) — ten sam rozdział ról co przy D7/D8/D10. Dostawy 9 i 10 mają rozdzielne nośniki: §12 to **specyfikacja do wykonania**, §13 to **rejestr do rozliczenia** — pozycje D7/D8/D10, które padają w obu, są w §12 opisane jako **wymóg wykonawczy**, a w §13 wyłącznie **zaksięgowane z progiem**. §8 pojawia się przy dostawie 9 jako jej część (teksty wiążące), a nie jako osobna dostawa. Sekcje 0, 6, 7 to konsekwencje wymagane przez rozstrzygnięcia (fakty nośne, przypadki brzegowe, ślad w danych), nie zlane dostawy.

### A2 — spójność: nic nie opiera się na tym, co wyłączone

**[NOWE v0.9] Druga forma pytania A2 — obowiązuje od tej wersji.** Dotąd A2 pytała: „czy element opiera się na czymś, co jest w OUT?". To pytanie **przepuściło** kartę wejścia w moduł: trzecia powierzchnia nie była w OUT — nie było jej **nigdzie**, więc nic nie zapalało się na czerwono. Dokładam pytanie drugie: **„czy każdy mechanizm z tego dokumentu ma NAZWANĄ powierzchnię albo nośnik, i czy ta nazwa pada w sekcji, która je wylicza?".** Sprzeczność §12.1 (dwie powierzchnie) z §12.4 (trzy) była widoczna wyłącznie przy tak postawionym pytaniu — i wyłapała ją Mila, nie moja brama.
...
| **[v0.9] Karta wejścia w moduł (§12.4 w brzmieniu v0.8)** | **mechanizm różnicujący wsparcie wg `supportMode`** | **BYŁA OPARTA NA CZYMŚ NIEISTNIEJĄCYM — nazwane i USUNIĘTE w v0.9:** `supportMode` nie ma czytelnika (pisarze: `placement.ts:422`, `placement-service.ts:411`; poza nimi nic). §12.4 zawężona do powierzchni B; teksty zostają w §8 bez powierzchni, pozycja zaksięgowana jako **D12** z progiem „pierwszy czytelnik `supportMode`". **Po tej zmianie §12.1 i §12.4 mówią tę samą liczbę powierzchni** |
| **[v0.9] Blok 3 jako link do modułu** | **założenie, że cel kariery jest w wierszu studenta przy renderze kroku 4** | **Założenie SPRAWDZONE i PRAWDZIWE** (`runSubmit` leci w `handleDiagnosisFinished`, przed `advanceTo(4)`) — link nie byłby martwy. Mimo to **blok 3 zostaje tekstem** z trzech powodów produktowych (§12.3); decyzja nie opiera się już na nieprawdziwej przesłance o martwym linku |
| **[v0.11] Odczyt miernika na produkcji** | **założeniu, że każde zdarzenie pochodzi od uczestnika pilotażu** | **ZAŁOŻENIE OBALONE PRZEZ PIERWSZY WIERSZ — nazwane w v0.11:** pierwsze zdarzenie pochodzi z weryfikacji zapłonu kontem QA i wygląda jak udany placement. Reguła odczytu przeniesiona z „mianownik z listy uczestników" (v0.7) na **licznik i mianownik** (§6a); wykluczenie NIE opiera się na atrybutach konta, bo zdarzenie nie ma `actorId`, a złączenie do konta znika kaskadą przy jego skasowaniu |
| **[v0.10] Tekst `below_threshold` przy niezerowych odblokowaniach** | **polu „kompetencja głębsza", którego reguła placementu NIE GWARANTUJE** | **BYŁ OPARTY NA CZYMŚ NIEISTNIEJĄCYM — nazwane i NAPRAWIONE w v0.10:** dziurą zostaje pierwszy otagowany moduł, który nie spełnia warunku, więc moduł zakwalifikowany głębiej może nie istnieć (`ds-python=4, ds-pandas=1` bez SQL) albo istnieć (`…, ds-sql=4`) — oba kształty osiągalne. Tekst przepisany na pola gwarantowane; kategoria „kompetencja głębsza" zamknięta na stałe (§8) |
| **[v0.10] Tekst korzenia w bloku 3** | **literałowi „Start: środowisko pracy" w §8** | **SPRZECZNOŚĆ Z WŁASNĄ REGUŁĄ — naprawiona w v0.10:** §12.8 pkt 2 zabrania mapy slug→tytuł po stronie klienta, bo tytuły są danymi z ingestu; mój własny tekst wpisywał tytuł literałem. Zamienione na interpolację z kontraktu |
| **[v0.10] Warianty zerowe §8 w wariancie 8 (zaliczenia > 0)** | **zdaniu „zaczynamy od początku ścieżki"** | **OPARTE NA NIEPRAWDZIE U TEGO STUDENTA — nazwane w v0.10, świadomie NIE naprawione:** render milczy (fail-closed), pozycja zaksięgowana jako **D13** z progiem i gotowym kształtem naprawy. Powód odłożenia: osiągalność, nie waga (§13/D13) |
| **[v0.9] Nagłówek sekcji na powierzchni A** | §8 (teksty wiążące) | **DZIURA ZAMKNIĘTA w v0.9:** nagłówek nie miał źródła w §8, więc powstał poza regułą „teksty 1:1" — nie z winy Mili, tylko dlatego, że §8 nie obejmował etykiet strukturalnych. Tekst dopisany do §8, reguła rozszerzona na nagłówki i `aria-label` (§12.3) |
...
### Self-critique v0.9 — pięć słabości rozstrzygnięć dla Mili i co z nimi zrobiłam

Rola krytyka: senior product lead z SaaS edukacyjnego, po launchu, w którym specyfikacja przeszła bramę własnego autora i dopiero wykonawca zauważył, że dwa jej rozdziały opisują inny produkt.

**1. „Twoja brama A1/A2 przepuściła sprzeczność §12.1 kontra §12.4 na zielono, a znalazła ją Mila. Brama, która sprawdza dostawy wobec listy zamawiającego, nie sprawdza dokumentu wobec samego siebie."** To nie był drobiazg: §12.4 opierał **dwa z dziewięciu powodów** na powierzchni, której mój własny zakres nie deklarował — czyli dokładnie ten kształt błędu, który A2 ma łapać, tylko odwrócony (element opierał się nie na tym, co w OUT, ale na tym, czego **nie ma nigdzie**). → **Zmienione:** A2 dostała **drugą formę pytania** („czy każdy mechanizm ma nazwaną powierzchnię i czy nazwa pada w sekcji, która je wylicza") — bo pierwsza forma z definicji nie mogła tego zobaczyć. Plus przyznanie wprost, że wyłapał to wykonawca, nie autor.

**2. „Odkładasz kartę, bo `supportMode` nie ma czytelnika — ale teksty, które ją miały wypełnić, napisałaś w v0.1 i v0.3 i broniłaś nimi trzeciej gałęzi trybu wsparcia. Sprawdziłaś kod dopiero wtedy, gdy ktoś zapytał o umiejscowienie."** Prawda i to niewygodna: przez pięć wersji nosiłam w §8 zdanie „zostawiamy pełne wsparcie włączone", nie sprawdziwszy ani razu, czy cokolwiek to wsparcie różnicuje. → **Zmienione:** D12 dostała **warunek domknięcia mocniejszy niż istnienie kolumny** — karta wchodzi dopiero, gdy tryb wsparcia różnicuje coś, co **student może zobaczyć**; sam mechanizm w bazie nie wystarcza. Przy okazji sprawdziłam uzasadnienie trzeciej gałęzi z DECYZJI 2 i potwierdzam: było od początku **danymi**, nie komunikatem, więc odłożenie karty jej nie osłabia. Gdyby było komunikatem — musiałabym cofać DECYZJĘ 2, nie odkładać kartę.

**3. „Prostujesz Mili fakt o martwym linku i na tym budujesz przewagę — ale sama zbudowałaś §12.6 wariant 1 na zdaniu »brak sesji diagnozy«, nie sprawdzając, że student może nie zobaczyć tego ekranu także po odświeżeniu w trakcie kroku 4."** Trafne. Opisywałam jednorazowość jako „znika po odświeżeniu **wyjaśnienie**", a prawda jest ostrzejsza: znika **cała sekcja**, bo wynik diagnozy żyje w stanie komponentu, a onboarding nie jest jeszcze domknięty. → **Zmienione:** §12.7 pkt 6 rozszerzony o ten przypadek, z wnioskiem, który zmienia wagę pracy: powierzchnia B jest jedynym miejscem, o którym **wiemy**, że student je zobaczy. To argument przeciw linkowi mocniejszy niż te, które wypisałam w §12.3 — i pasuje do nich, zamiast im przeczyć.

**4. „Nagłówek »Po diagnozie« obalasz cudzy i stawiasz własny, po czym w tym samym akapicie tłumaczysz, dlaczego jest niedokładny nad blokiem 1. To rozstrzygnięcie z wbudowanym wyjątkiem."** Uczciwie: tak. Żaden z rozważanych nagłówków nie był poprawny we wszystkich dziesięciu wariantach bez zastrzeżenia — „Twoja ścieżka" łamie §12.7 pkt 1, rodzina „punkt startu" jest zamknięta przez §7, wariant nazywający diagnozę podporządkowuje jej pracę studenta wbrew §12.10, a wariant nazywający jedno i drugie rozpada się w wariancie 7 (brak zaliczeń). → **Zmienione:** zamiast udawać wybór bez kosztu, **nazwałam koszt w §8** i oznaczyłam nagłówek jako **pierwszy do rewizji z rozmów pilotażowych**, z kosztem zmiany „jedna stała tekstowa". Rozstrzygam, ale nie pozoruję, że rozstrzygnięcie jest darmowe.

**5. „Dokładasz D12 do rejestru, którego pierwszą pozycję sama opatrzyłaś zdaniem »tak umierają wszystkie długi zapisane w dokumentach« — a D12 przypinasz do zdarzenia, którego nie ma w żadnym planie."** Próg „pierwszy czytelnik `supportMode`" jest uczciwy merytorycznie i bezużyteczny operacyjnie, jeśli wygaszanie wsparcia nigdy nie powstanie. → **Zmienione:** D12 dostała **drugi, bezwarunkowy termin** — przy pierwszej rewizji §8 (przychodzi z D1/D2) oba teksty albo dostają powierzchnię, albo **znikają z §8**. To warunek samoczyszczący: mikrocopy bez powierzchni wolno trzymać przez jedną rewizję jako gotowe wejście, dłużej zaczyna być treścią, którą kolejny wykonawca wstawi gdziekolwiek, bo „jest w §8".

### Self-critique v0.10 — pięć słabości i co z nimi zrobiłam

Rola krytyka: senior product lead z SaaS edukacyjnego, po launchu, w którym mikrocopy przeszła trzy przeglądy i dopiero implementacja pokazała, że jedno zdanie nie miało prawa się wyrenderować.

**1. „Twój tekst `below_threshold` przetrwał dziewięć wersji, dwie bramy A1/A2 i trzy przebiegi samokrytyki — a nie dało się go wyrenderować. Nie sprawdziłaś ani razu, czy pola, których żądasz, w ogóle powstają."** W v0.8 dopisałam wariant zerowy dokładnie dlatego, że pola nie istnieją przy zerze odblokowań — i **zatrzymałam się na tym**, nie pytając, czy „kompetencja głębsza" istnieje przy niezerowych. Miałam już wtedy sygnał: w self-critique v0.8 napisałam wprost, że to pole „też nie zawsze istnieje", po czym nie wyciągnęłam z tego wniosku dla drugiego wariantu. → **Zmienione:** tekst przepisany na pola gwarantowane przez regułę, a **reguła interpolacji rozszerzona ze „sesji zerowych" na wszystkie teksty o dziurze**, z zamknięciem kategorii „kompetencja głębsza" na stałe. Naprawiam klasę, nie zdanie — bo zdanie naprawiłam już raz i nie wystarczyło.

**2. „Wybrałaś (B) i nazywasz (A) »niebezpiecznym« — ale (A) działa w części sesji, więc dałoby wyjaśnienie tam, gdzie dziś jest cisza. Nie sprzedajesz sobie własnego wyboru?"** Uczciwie sprawdzone: (A) dałoby pełniejsze zdanie w sesjach z głębszym kwalifikantem (`…, ds-sql=4`), a w pozostałych **zostawiłoby tę samą ciszę**. Czyli kosztem zmiany kontraktu przed samym scaleniem kupiłabym **zwężenie** ciszy i dwie klasy studentów w tej samej sytuacji. → **Zmienione:** argument w §8 przeformułowany z „drogie" na to, co jest naprawdę rozstrzygające — **(A) nie usuwa ciszy, tylko ją dzieli**, a w drugim kształcie danych to samo pole niesie moduł *płytszy* niż dziura i zdanie o fundamencie staje się nieprawdą o naszej własnej drabinie.

**3. „Ratyfikujesz sześć rozstrzygnięć wykonawcy hurtem — to jest pieczątka, nie przegląd."** Zagrożenie realne, bo wszystkie sześć brzmi rozsądnie. → **Zmienione:** każde przeszło osobno i **dwa z nich okazały się naprawami MOICH błędów**, nie decyzjami Jacka: literał tytułu w tekście korzenia (łamał moją regułę z §12.8 pkt 2) i zawężenie `deepestUnlockedTitle` do jednego wariantu renderu. Jedno **odrzuciłam** w warstwie, w której padło (etykieta czytnika ekranu — ale zapisałam skutek produktowy u siebie, nie w komentarzu w kodzie), a jedno **przekwalifikowałam** z „zapisz" na **dług z progiem** (D13), bo cisza w wariancie 8 to nie ustalenie techniczne, tylko odłożona decyzja produktowa.

**4. „Zamykasz D13 zdaniem »w pilotażu nieosiągalne« — tym samym argumentem, którym w v0.9 uzasadniałaś odłożenie karty wejścia, i tym samym, który sama obaliłaś jako fałszywy przy populacji `qualified`."** Trafne i dlatego sprawdziłam osiągalność wariantu 8 wprost, zamiast oszacować: wymaga **koniunkcji czterech warunków**, w tym ponownej diagnozy, która istnieje wyłącznie przez re-onboarding (§6b). To inny kaliber niż „`qualified` na F1", które wymaga **jednego** trafionego pytania z najczęściej zaznaczanej kompetencji. → **Zmienione:** w D13 zapisałam **różnicę osiągalności jako liczbę warunków**, nie jako wrażenie, i dołożyłam gotowy kształt naprawy, żeby odłożenie nie oznaczało wymyślania jej od nowa.

**5. „Rozstrzygasz interpretację kroku 3 algorytmu, o którą nikt Cię nie prosił w zleceniu — i przyznajesz, że skutkiem jest złamanie DECYZJI 3. Właśnie cofnęłaś własną decyzję przy okazji, w akapicie o czymś innym."** Zarzut poważny, bo DECYZJA 3 chroni przed najtańszym do uniknięcia porzuceniem. Sprawdziłam: nie cofam jej — nadpisanie korzeniem działa **zawsze, gdy korzeń da się zarekomendować**; wyjątek dotyczy wyłącznie korzenia **bez treści**, czyli stanu, który przy czterech pozycjach `l0-start` z mocy D10 oznacza **błąd ingestu**. → **Zmienione:** konsekwencja nazwana wprost w §12.2 razem ze wskazaniem właściwej naprawy (ingest, nie rekomendacja), zamiast zostawienia jej jako cichego skutku ubocznego interpretacji. Gdyby korzeń mógł być pusty w normalnej pracy, rozstrzygnięcie byłoby odwrotne.

### Self-critique v0.11 — pięć słabości zapisu o pierwszym wierszu i co z nimi zrobiłam

Rola krytyka: senior product lead z SaaS edukacyjnego, po launchu, w którym pierwsze dane z produkcji były w połowie własnymi przebiegami zespołu i nikt nie zauważył tego przez kwartał.

**1. „Dostałaś gotowy rozróżnik — domenę `.invalid` — i mogłaś go po prostu zapisać. Sprawdzanie, czy zdarzenie w ogóle ma czym dosięgnąć konta, nie było zamówione."** Gdybym zapisała rozróżnik podany w zleceniu, dokument miałby regułę, która **działa dziś i cicho przestaje działać** w dniu skasowania konta QA — a wtedy wiersz techniczny wraca do licznika już nie do odróżnienia. → **Zmienione:** sprawdziłam `recordPlacementMetricEvent` i schemat `audit_log`; zdarzenie **nie ma `actorId`** z mocy wymogu Ryana (art. 17 RODO), więc do konta dociera się wyłącznie złączeniem znikającym kaskadą. Reguła podstawowa jest w efekcie **niezależna od konta**: przypisanie do sesji nazwanego uczestnika albo brak obserwacji. Domena `.invalid` zostaje jako rozróżnik **pomocniczy**, z nazwanym terminem ważności.

**2. „Rozwiązałaś to regułą, a nie faktem — a reguła bez wpisanego identyfikatora jest zaproszeniem do »chyba to nie ten wiersz«."** Zarzut trafny w drugą stronę niż mój wybór, więc rozważyłam wpisanie identyfikatora sesji i znacznika czasu. Odrzuciłam: nie mam tych wartości (są na produkcji, a nie zaglądam tam z tej sesji), a **puste miejsce „do uzupełnienia" to dokładnie ten dług, który opisałam w D5** — nikt go nie wypełni. → **Zmienione:** reguła sformułowana tak, żeby **nie potrzebowała żadnej wartości** (przypisanie do imiennej listy uczestników), plus dwa rozróżniki pomocnicze do sprawdzenia ręcznego. Efekt uboczny, którego nie planowałam, a który jest cenniejszy niż sam zapis: reguła pokrywa też **każdą przyszłą** odegraną sesję — weryfikację po wdrożeniu, demo dla design partnera, szkolenie.

**3. „Rozszerzasz zasadę z mianownika na licznik i nazywasz to rozszerzeniem — a to jest przyznanie, że Twoja zasada z v0.7 była za wąska i nie zauważyłaś tego przez cztery wersje."** Tak. W v0.7 napisałam „mianownik bierzemy z listy uczestników, nie z dziennika", bo widziałam jedno ryzyko: studenta, który **nie zostawia** zdarzenia. Nie zobaczyłam symetrycznego: zdarzenia, które **nie ma studenta**. → **Zmienione:** reguła zapisana raz, dla obu stron ułamka, z jawnym wskazaniem, że zanieczyszczenie trafiło w licznik. Zapisuję też, że pierwsza wersja była połową reguły — bo inaczej następna asymetria przejdzie tak samo.

**4. „Notujesz dwa potwierdzenia z produkcji jako sukces — a to jest dokładnie ten typ zdania, który sama zakazujesz czytać jako obserwację."** Uczciwie: „nadpisanie korzeniem działa na prodzie" i „dziura wypadła na `no_measurement`" **kuszą** jako pierwsze dowody wartości funkcji. → **Zmienione:** oba zapisane w §6a jawnie jako **dowód o kodzie, nie o studentach**, z jednozdaniowym powodem: wejścia tej sesji wybrał człowiek, który wiedział, co chce zobaczyć. Dopisałam trzecie potwierdzenie, którego nikt nie zgłosił (kształt przebiegu zgadza się z przewidywaniem §6a pkt 4 — Python zdany, dziura na Pandas), i **jego też** opatrzyłam tym samym zastrzeżeniem: potwierdza spięcie mapy tagów, reguły i ekranu end-to-end, i nic ponadto.

**5. „Piszesz »ten wiersz wygląda jak udany placement« i zostawiasz to jako ostrzeżenie — ale nie mówisz, ile realnie waży przy n=1–3."** Waży maksymalnie: przy trzech uczestnikach jedna odegrana sesja to **25% wszystkich zdarzeń**, a jeśli uczestnicy okażą się „literalnym zerem" (czyli zgodnie z personą nie odblokują nic), to **jedyne** zdarzenie z niezerowym odblokowaniem w całym pilotażu będzie tym technicznym. → **Zmienione:** ta konsekwencja nazwana wprost w §6a („stuprocentowa skuteczność zbudowana w całości na odegranej sesji") zamiast ogólnego „uważaj" — bo dopiero z liczbą widać, że to nie jest drobiazg porządkowy, tylko jedyny wynik, jaki pilotaż może pokazać.
...
| Mila (Design) | warstwa wizualna ekranu wyniku diagnozy (§12.10) | **projekt oddany i przyjęty; trzy pytania rozstrzygnięte w v0.9 — trzy poprawki do naniesienia (§12.10)** |
| Jack (Frontend) | implementacja §12 — **dwie** powierzchnie (karta wejścia w moduł POZA zakresem, D12), dziesięć wariantów renderu, mapa dziewięciu powodów | **ekran oddany; siedem rozjazdów zgłoszonych — sześć ratyfikowanych, jeden (tekst `below_threshold`) naprawiony w §8. Do naniesienia: jedna zmiana tekstu + zdjęcie ciszy w tej jednej gałęzi** |
