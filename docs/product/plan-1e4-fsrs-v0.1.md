...
- **Wejście do harmonogramu:** koncept **kluczowy** modułu wchodzi, gdy student **zda mastery gate** tego modułu. Nie po zaliczeniu atomu (*zaliczony ≠ opanowany*, ADR-014 D3). → **decyzja otwarta D1, patrz §5**.
- **Definicja „konceptu kluczowego" (rozstrzygnięcie R5, Sophia):** koncept aktywny (`question_concepts.status='active'`) o `trunk='market'` (na liściu kompetencji rynkowej), zmapowany na dowolną pozycję modułu. Koncepty `foundations` (trzon CS/matmy) **nie wchodzą** do kolejki — obietnica „powtórki pilnują kruchego" chroni kompetencję rynkową (to, co widzi pracodawca w Paszporcie), a fundamenty są utrwalane implicytnie pracą nad kompetencją rynkową. Rozmiar kolejki „na dziś" throttluje twardy dzienny cap nowych kart (**D3**, §5) — `trunk='market'` mówi KTÓRE koncepty, D3 ile ich naraz trafia do kolejki. Mechanizm „≤4/moduł tagowane przy autoringu" (v0.4) **wycofany**: kolumny key/weight/tag w schemacie nie ma, a `trunk='market'` + cap D3 realizują tę samą intencję humanitarnej kolejki na istniejącej, CHECK-owanej kolumnie (Built-to-Sell: kupujący czyta filtr wprost). Kontrakt-test pojemności (§6: koncept bez ≥1 pytania nie wchodzi) obowiązuje wewnątrz tego zbioru bez zmian.
...
- **Sprzężenie z 1E.3 — DEFER poza R5 (rozstrzygnięcie R5, Sophia).** „Niższa stabilność początkowa dla konceptu oblanego na egzaminie" (`failedConcepts`) **nie wchodzi do R5** — enrollment R5 jest prosty: wszystkie zdane koncepty kluczowe wchodzą jednakowo. Powód produktowy: ts-fsrs re-inicjalizuje stabilność/trudność przy **pierwszej realnej ocenie**, więc sprzężenie zmieniałoby tylko moment pierwszego wypłynięcia — a kruchy koncept i tak obleje 1. powtórkę i FSRS ściągnie go szybko z powrotem (obietnica „powtórki pilnują kruchego" trzyma się na samym FSRS, nie na tym biasie). Defer jest **odwracalny i bez straty sygnału**: `failedConcepts` jest już trwale w 1E.3, więc przyszła iteracja ma dane. Kierunek iteracji: **„krótszy pierwszy `due` dla oblanych"** (nudge tylko na moment pierwszego wypłynięcia), **NIGDY** syntetyczny wpis do `review_logs` (złamałby append-only + `question_item_id NOT NULL` = uszkodzenie śladu Built-to-Sell). Patrz §7 „Iteracja po MVP".
- **Relearning po potknięciu — zostaje w R5.** Potknięcie w powtórce → koncept wraca do krótkich odstępów (relearning), nigdy „oblany na stałe". To czyste zachowanie ts-fsrs (nie sprzężenie z 1E.3), niezależne od powyższego defera.
...
- **K5 (R5) — `priorFailed` coupling = ROZSTRZYGNIĘTE (Sophia, przegląd produktowy R5): DEFER.** Sprzężenie zasiewu z sygnałem „już oblany" (hook `priorFailed`, dziś no-op) **nie wchodzi do R5** — hook zostaje no-op, enrollment jednakowy dla wszystkich zdanych konceptów kluczowych. Argument techniczny Ethana przyjęty (re-inicjalizacja S/D przy 1. ocenie; zakaz syntetycznego `review_log`). Iteracja po MVP jako „krótszy pierwszy `due`" — §2 (sprzężenie z 1E.3) i §7 „Iteracja po MVP". Odblokowuje R5 dla Maxa: `priorFailed` zostaje pustym hookiem, bez nowej migracji i bez wpięcia w zdarzenie oblania.
...
### 4d. Carry-forward z R5 → do rozwiązania w R6

R5 (hook enrollment powtórek w zdany mastery gate, owner-side, flaga OFF) scalona z Leo GO warunkowym (W1 = test integracyjny Quinna, zamknięty; DSN CI zachowany). Trzy pozycje przeniesione do R6 — **świadomie NIE rozwiązane w R5** (dług śpi przy fladze OFF; brak realnego ruchu = brak konsekwencji dziś):

- **N1 — filtr pojemności type-agnostyczny → ryzyko phantom-wiersza (owner decyzji: Sophia/Ethan).** Filtr pojemności w `enroll-hook.ts` (koncept wchodzi do zasiewu tylko gdy ma ≥1 aktywne pytanie) używa `EXISTS` po **dowolnym** typie pytania. Dla konceptu `trunk='market'` aktywnego, który ma ≥1 pytanie, ale **zero aktywnych `single_choice`**, powstałby wiersz `review_states` (phantom) bez materiału, którym warstwa powtórki (R4, `single_choice`) mogłaby go obsłużyć. Decyzja do podjęcia w R6: **zawęzić `EXISTS` do `single_choice`** (parytet z R4 — warstwa powtórki serwuje dziś tylko ten typ) albo **świadomie zaakceptować** (jeśli R6 rozszerzy typy powtórki). Dług śpi: dziś nie ma market-pytań typu innego niż `single_choice`, więc phantom-wiersz nie może dziś powstać.
- **N2 — indeks pod korelowany `EXISTS` (owner: Ethan/Max).** Potwierdzić istnienie albo dodać indeks `question_items(concept_id, status)` pod skorelowany podzapytaniem `EXISTS` filtra pojemności (N1) — żeby przy zapłonie flagi i realnej liczbie konceptów zapytanie enrollmentu nie robiło pełnego skanu. Weryfikacja przy R6 (migracja indeksu, jeśli brak) — dodanie indeksu jest addytywne i nie dotyka danych.
- **N3 — `after()` (Next 15) dla rozprzężenia zasiewu (owner: Ethan).** Rozważyć przeniesienie best-effort zasiewu powtórek do `after()` (API Next.js 15 do pracy po odesłaniu odpowiedzi) — dziś hook jest wołany po transakcji egzaminu, ale wciąż w cyklu żądania, więc dokłada się do latencji ogona (`p95`) zdanego egzaminu. `after()` odprzęgłoby zasiew od odpowiedzi trasy. Nie blokuje R6, ale poprawia p95 przy zapłonie flagi; do zmierzenia na realnym ruchu.
...
Samoocena studenta (Again/Hard/Good/Easy) · nowa treść pytań powtórkowych · powiadomienia mail/push · trenowanie własnych wag · grywalizacja/streak · multi-tenant skala · **sprzężenie „oblany → niższa stabilność początkowa" (§2, DEFER)** · **corrective-done jako wyzwalacz enrollment (patrz niżej)**. Wszystko za wzorcem 1E.3 — wąsko, flaga OFF, iterujemy na danych pilotażu.

### 7a. Iteracja po MVP — „harmonogram świadomy kruchości" (fragility-aware scheduling)

Jeden spójny wątek, świadomie odłożony z R5, do podjęcia po pierwszych danych pilotażu (gdy zobaczymy, czy domyślny moment pierwszej powtórki jest realnie za późny dla konceptów kruchych). Dwa powiązane wyzwalacze:

1. **Krótszy pierwszy `due` dla konceptów oblanych na egzaminie** (`failedConcepts`, §2). Nudge tylko na moment pierwszego wypłynięcia; po pierwszej realnej ocenie matematykę przejmuje FSRS. Implementacja **wyłącznie** jako skrócenie pierwszego `due`, **nigdy** jako syntetyczny wpis do `review_logs` (kontrakt append-only + `question_item_id NOT NULL` = ślad Built-to-Sell; syntetyczna ocena go łamie). Bez nowej migracji.
2. **Corrective-done jako wcześniejszy moment enrollment** (Decyzja 3 przeglądu R5, informacyjnie). Koncept oblany-a-zremediowany to najsłabsza pamięć = największy zysk z powtórek — Ethan słusznie to zgłasza. **Ale w R5 te koncepty i tak wchodzą** przez mastery gate (student po correctives zdaje egzamin → koncept enrolluje się przy zdaniu bramki). Corrective-done zmieniałby tylko MOMENT (wcześniej, przy ukończeniu correctives, zanim padnie zdany egzamin) i wymagałby wyzwalacza czasowego/hooka na `correctivesDoneAt` — nowej infrastruktury zdarzeń poza zakresem R5. Dlatego: **wejście tylko przez mastery gate w MVP**; corrective-done rozważamy razem z (1) jako część tego samego wątku „wcześniejsze/częstsze wypłynięcie konceptu kruchego", dopiero gdy dane pilotażu pokażą, że timing enrollmentu jest realnym problemem retencji.
