...
// onboardingCompleted=true. Guard: profil realny (university != "") + >=5
// kompetencji. Inaczej 409 (nie domyka onboardingu na prowizorycznym rekordzie).
//
// Test REALNY na bazie :5433 (mock tylko auth). Dwa przypadki:
//  (1) realny profil + 5 kompetencji → 200 {success, redirect:"/dashboard"}, completed=true
//  (2) placeholder (university="") → 409, completed zostaje false
...
dBack("POST /api/onboarding/complete (realna baza)", () => {
	...
	async function cleanupUsers() {
		if (!pool) return;
		for (const uid of [USER_OK, USER_PLACEHOLDER]) {
